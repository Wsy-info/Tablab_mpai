...
### get all degs
deg1 <- get_degs(heart.harmony, "Young")
deg2 <- get_degs(heart.harmony, "MET")
deg3 <- get_degs(heart.harmony, "NR")
deg4 <- get_degs(heart.harmony, "D+Q")
deg5 <- get_degs(heart.harmony, "SPD")
deg6 <- get_degs(heart.harmony, "MET+NR")
deg7 <- get_degs(heart.harmony, "MET+SPD")
...
names(list1) <- cluster_order
capture.output(list1, file = "Heart_Pro-aging_genes_1.csv")
...
names(list2) <- cluster_order
capture.output(list2, file = "Heart_Pro-aging_genes_2.csv")
...
ggplot(data1, aes(x = Var1, y = ifelse(group == "Rev-aging", value, -value), fill = Var2)) +
  ...
  labs(x = "", y = "Gene number", title = "The number of Rev-aging and Pro-aging DEGs in Heart") +
