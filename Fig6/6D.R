...
names(list1) <- cluster_order
capture.output(list1, file = "Kidney_Pro-aging_genes_1.csv")
...
names(list2) <- cluster_order
capture.output(list2, file = "Kidney_Pro-aging_genes_2.csv")
...
ggplot(data1, aes(x = Var1, y = ifelse(group == "Rev-aging", value, -value), fill = Var2)) +
  ...
  labs(x = "", y = "Gene number", title = "The number of Rev-aging and Pro-aging DEGs in Kidney") +
