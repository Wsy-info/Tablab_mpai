...
### get all degs
deg1 <- get_degs(liver.harmony, "Young")
deg2 <- get_degs(liver.harmony, "MET")
deg3 <- get_degs(liver.harmony, "NR")
deg4 <- get_degs(liver.harmony, "D+Q")
deg5 <- get_degs(liver.harmony, "SPD")
deg6 <- get_degs(liver.harmony, "MET+NR")
deg7 <- get_degs(liver.harmony, "MET+SPD")
...
names(list1) <- cluster_order
capture.output(list1, file = "Liver_Pro-aging_genes_1.csv")
...
names(list2) <- cluster_order
capture.output(list2, file = "Liver_Pro-aging_genes_2.csv")
...
ggplot(data1, aes(x = Var1, y = ifelse(group == "Rev-aging", value, -value), fill = Var2)) +
  ...
  labs(x = "", y = "Gene number", title = "The number of Rev-aging and Pro-aging DEGs in Liver") +
